...
class InputValidator {
    ...
    static checkWasmModuleName(value) {
        if(value === undefined || value === '') {
            return 'The Wasm Module name must be specified.'
        }

        //1.remove all space
        value = value.replace(/\s*/g, '');

        //2. change to lower character
        value = value.toLowerCase();

        //3. check length, <= 20
        if(value.length > 20) {
            return 'The Wasm Module Name should lower than 20';
        }

        //4. not repeat
        //4.1 get all wasm module name list
        const workspaceRootPath = vscode.workspace.rootPath;
        let wasmModulePath = path.join(workspaceRootPath, 'wasm_modules');

        if(!fs.existsSync(wasmModulePath)) {    //not exist
            console.log(wasmModulePath + 'not exist!!');
            return null;
        }
        //4.2 traverse list
        ...
        for(let dirname of dirs)
        {
            //console.log(dirname);
            if(value === dirname) {
                return 'The Wasm Module Name is already exist!!';
            }
        }
