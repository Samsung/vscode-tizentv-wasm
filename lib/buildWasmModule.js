...
async function buildEachWasmModule(buildModule, buildMode, cCmd, cppCmd, linkCmd) {
    ...
    try {
        ...
        if(buildMode === 'Debug') {
            linkerCmd = `${tmpCmd} emcc -s EXPORT_NAME=${buildModule.wasmModuleName} -s MODULARIZE=1 ${linkCmd} -g4 --bind -o ${buildModule.wasmModuleName}.js `;
        } else if(buildMode === 'Release') {
            linkerCmd = `${tmpCmd} emcc -s EXPORT_NAME=${buildModule.wasmModuleName} -s MODULARIZE=1 ${linkCmd} -O3 --bind -o ${buildModule.wasmModuleName}.js `;
        }
        ...
        for(let file of buildModule.builtFiles) {
            ...
            if(file.indexOf('.cpp') != -1) {
                //console.log('Invoking : Emscripten C++ Compiler');
                logger.log(moduleID, 'Invoking : Emscripten C++ Compiler');

                if(buildMode === 'Debug') {
                    compilerCmd = `${cmd} emcc -O0 -g4 -Wall -c -fmessage-length=0 ${cppCmd} -I../inc -std=gnu++11 --bind`;  //cpp debug
                } else if(buildMode === 'Release') {
                    compilerCmd = `${cmd} emcc -Oz -O3 -Wall -c -fmessage-length=0 ${cppCmd} -I../inc -std=gnu++11 --bind`;   //cpp release
                }
