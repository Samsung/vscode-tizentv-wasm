...
module.exports = async function buildWasmModule() {
    ...
    //wasmLog.wasmLog(moduleID, 'buildMode = ' + buildMode);
    ...
    if(builtFiles === null || builtFiles.length === 0) {
        ...
        //wasmLog.wasmLog(moduleID, `No wasm module should be built!!`);
    ...
    for(let buildModule of builtFiles) {
        ...
        //wasmLog.wasmLog(moduleID, '----------------------------------------Build Wasm Module : ' + buildModule.wasmModuleName + ' ----------------------------------');
        ...
        //wasmLog.wasmLog(moduleID, '----------------------------------------Finished building Wasm Module : ' + buildModule.wasmModuleName + ' ----------------------------------');
    ...
    //wasmLog.wasmLog(moduleID, 'build all wasm module success!!');
...
function _execAsync(cmd, outCheck) {
    return new Promise(function(resolve, reject) {
        ...
        //wasmLog.wasmLog(moduleID, `Build Wasm Module: ${cmd}`);
...
async function buildEachWasmModule(buildModule, buildMode, cCmd, cppCmd, linkCmd) {
    ...
    try {
        ...
        //wasmLog.wasmLog(moduleID, '**** Build of configuration ' + buildMode + ' for project ' + buildModule.wasmModuleName + ' ****');
        //wasmLog.wasmLog(moduleID, 'make all');
        ...
        for(let file of buildModule.builtFiles) {
            ...
            if(file.indexOf('.cpp') != -1 || file.indexOf('.cc') != -1) {
                ...
                //wasmLog.wasmLog(moduleID, 'Invoking : Emscripten C++ Compiler');
        ...
        //wasmLog.wasmLog(moduleID, 'Building target : ' + buildModule.wasmModuleName + '.js');
        //wasmLog.wasmLog(moduleID, 'Invoking: Emscripten Linker');
        ...
        //wasmLog.wasmLog(moduleID, 'Finished building target : ' + buildModule.wasmModuleName + '.js')
